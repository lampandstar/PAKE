...
#if 0//This requires CPUID "SHA", unfortunately my CPU does not support yet
#define UPDATE(data)\
	do {\
		ABEF_SAVE = STATE0;\
        CDGH_SAVE = STATE1;\
...
void sha256(OUT uint8_t dgst[32], IN uint8_t * msg, IN uint64_t len)
{
	...
	int64_t blocks = (len + 63) >> 6;
	uint8_t block[64] = {0}, * p = msg;
	
	
	__m128i STATE0, STATE1 = _mm_setr_epi32(0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a);
    __m128i MSG, TMP = _mm_setr_epi32(0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19);
    __m128i MSG0, MSG1, MSG2, MSG3, *K = (const __m128i *)k;
    ...
    /* Save state */
    _mm_storeu_si128((__m128i*) dgst, STATE0);
    _mm_storeu_si128((__m128i*) (dgst+16), STATE1);
}
#endif